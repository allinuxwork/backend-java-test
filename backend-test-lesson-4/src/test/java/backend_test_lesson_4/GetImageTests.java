...
import static backend_test_lesson_4.steps.CommonRequests.uploadCommonImage;
...
public class GetImageTests extends BaseTest {
	...
	static void createSpecs() {
		...
		withAuthReqSpec = new RequestSpecBuilder().addHeader("Authorization", token).log(LogDetail.ALL).build();
	...
	void getImageInfoPositiveTest() {
		given().spec(withAuthReqSpec).log().all().when().get(Endpoints.GET_IMAGE_REQUEST, imgid).prettyPeek().then()
	...
	void tearDown() {
		given().headers("Authorization", token).when().delete(Endpoints.DELETE_IMAGE_REQUEST, username, imgid)
